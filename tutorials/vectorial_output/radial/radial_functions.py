...
def GPR_pretty_print(X,
    ...
    if n_train != 0:
        ...
        Y = Z.dot(ss.W1)
        y = (Y[:, 0]).reshape(-1, 1)
        ac.utils.plotters.sufficient_summary_3D(X_train, f[:n_train].reshape(
            (-1, )))
        ac.utils.plotters.eigenvalues(ss.eigenvals[:10])

        gp_NAS = ac.utils.gaussian_processes.GPRy(X_train[:, 0].reshape(-1, 1),
                                                  f[:n_train],
                                                  kernel=kernel)
        ...
        gp_NAS = ac.utils.gaussian_processes.GPRy(X_train[:, 0],
                                                  f[:n_train],
                                                  kernel=kernel)
        gp_NAS.plot(title='{} NAS'.format(title))
