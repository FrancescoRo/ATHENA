...
f = np.vstack((output, output2, output3))[:, 1].reshape(-1)  #Cd
...
GPR_AS = Estimator(sstype='AS',
                   ...
                   plot=True,
                   gp_dimension=dim)
mean_AS, std_AS = cross_validation(inputs=x,
                                   outputs=f,
                                   gradients=df,
                                   estimator=GPR_AS,
                                   folds=folds)
file = open("Lift.txt", 'a')
...
def tune_distr(namedDistr):
    ...
    params_opt, val_opt = tune(inputs=x,
                               outputs=f,
                               gradients=df,
                               n_features=D,
                               feature_map=fm,
    ...
    file = open("Lift.txt", 'a')
    file.write(
        "Best params for {0} are\n{1} corresponding NRMSE is {2}\n".format(
            name, params_opt, val_opt))
    file.close()
